...
exports.handler = async () => {
  ...
  try {
    ...
    const [destinasi, paket_wisata, transaksi] = await sql.begin(async sql => [
      await sql`SELECT id as id, nama as nama, harga FROM destinasi ORDER BY nama_destinasi ASC`,
      await sql`SELECT id_paket as id, nama_paket as nama, harga FROM paket_wisata ORDER BY nama_paket ASC`,
